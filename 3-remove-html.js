...
async function subHtml(request) {
	return new Response('优选订阅生成器，请使用 /sub 订阅接口', {
		headers: {
	...
	async fetch(request, env) {
		...
		noTLS = env.NOTLS || noTLS;
			...
			const responseBody = uniqueAddresses.map(address => {
				...
				if (协议类型 == 'VMess') {
					const vmessLink = `vmess://${utf8ToBase64(`{"v":"2","ps":"${addressid + 节点备注}","add":"${address}","port":"${port}","id":"${uuid}","aid":"${额外ID}","scy":"${加密方式}","net":"ws","type":"${type}","host":"${伪装域名}","path":"${最终路径}","tls":"tls","sni":"${sni}","alpn":"${encodeURIComponent(alpn)}","fp":"chrome","allowInsecure":"${scv == 'true' ? '1' : '0'}"}`)}`;
					return vmessLink;
				} else if (协议类型 == atob('VHJvamFu')) {
					const 特洛伊Link = `${atob(atob('ZEhKdmFtRnVPaTh2')) + uuid}@${address}:${port}?security=tls&sni=${sni}&alpn=${encodeURIComponent(alpn)}&fp=chrome&type=${type}&host=${伪装域名}&path=${encodeURIComponent(最终路径) + (scv == 'true' ? '&allowInsecure=1' : '')}#${encodeURIComponent(addressid + 节点备注)}`;
					return 特洛伊Link;
				} else {
					const 为烈士Link = `${atob(atob('ZG14bGMzTTZMeTg9')) + uuid}@${address}:${port}?security=tls&sni=${sni}&alpn=${encodeURIComponent(alpn)}&fp=chrome&type=${type}&host=${伪装域名}&path=${encodeURIComponent(最终路径) + xhttp + (scv == 'true' ? '&allowInsecure=1' : '')}&encryption=none#${encodeURIComponent(addressid + 节点备注)}`;
					return 为烈士Link;
				}
